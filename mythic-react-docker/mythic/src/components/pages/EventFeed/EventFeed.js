...
query GetOperationEventLogs($operation_id: Int!, $offset: Int!) {
  operationeventlog(where: {operation_id: {_eq: $operation_id}, deleted: {_eq: false}}, order_by: {id: desc}, limit: 10, offset: $offset) {
    id
    level
    message
    resolved
...
function EventFeedFunc(props){
  ...
  useEffect( () => {
      getInitialEvents({variables: {operation_id: me.user.current_operation_id, offset: 0}})
  }, [])
